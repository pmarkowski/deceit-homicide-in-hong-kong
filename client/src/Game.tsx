...
export const Game: FC<GameProps> = (props) => {
    return <div className="flex flex-col min-h-screen">
        ...
        <div className="my-12">
            <p className="text-light text-2xl mb-8">{props.forensicScientist.username} | Forensic Scientist</p>
            <div className="flex-row mb-12">
                {props.sceneCards.map(card => <SceneCardComponent key={card.title} {...card} />)}
            </div>
        </div>
        <div>
            {props.investigators.map(investigator => <InvestigatorComponent key={investigator.playerId} {...investigator} />)}
        </div>
    </div>
};
